...
const upload = multer({
  ...
  limits: {
    fileSize: 4 * 1024 * 1024, // 4MB limit to be safe on serverless
  },
...
export async function registerRoutes(app: Express, options?: { enableWebSockets?: boolean }): Promise<Server> {
  const httpServer = createServer(app);
  const enableWebSockets = options?.enableWebSockets ?? true;

  // WebSocket server for real-time updates (disabled on serverless)
  const wss = enableWebSockets ? new WebSocketServer({ server: httpServer, path: '/ws' }) : null;
  const clients = new Map<string, WebSocket>();

  if (wss) wss.on('connection', (ws: WebSocket, req) => {
  ...
  const broadcastQueueUpdate = (salonId: string, queueData: any) => {
    if (!wss) return; // No-op when websockets are disabled (e.g., Vercel serverless)
